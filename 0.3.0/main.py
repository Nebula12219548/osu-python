...
def about_screen(screen, clock):
    ...
    while running:
        ...
        version_text = version_font.render('v0.3.0', True, (180, 180, 180))
...
def maps_menu(screen, clock):
    ...
    while running:
        current_width, current_height = screen.get_size()
        draw_gradient_rect(screen, (0, 0, current_width, current_height), (30, 60, 120), (10, 10, 40), vertical=True)
        title = font_big.render('Select a Map', True, (0, 180, 255))
        title_rect = title.get_rect(center=(current_width//2, 80))
        screen.blit(title, title_rect)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                for i, opt in enumerate(options):
                    text_surface = font_medium.render(opt['label'], True, (255,255,255))
                    rect = text_surface.get_rect(center=(current_width//2, 180 + i*60))
                    if rect.inflate(40, 10).collidepoint(event.pos):
                        return opt['path']
        clock.tick(FPS)
...
def play_game(screen, clock, map_filepath, map_name):
...
def main_menu(screen, clock, settings):
    ...
    while running:
        ...
        for i, opt_label in enumerate(options):
            ...
            text_surface = font_medium.render(opt_label, True, color)
            rect = text_surface.get_rect(center=(current_width//2, 220 + i*60))
...
def main():
    ...
    while running:
        ...
        if selected_map_path:
            ...
            result = play_game(screen, clock, selected_map_path, map_name)
...
if __name__ == '__main__':
    # Define current_audio_temp_file outside main to be accessible globally if needed,
    # or handle its scope appropriately within play_game and main
    current_audio_temp_file = None 
    main()
    pygame.quit()
    sys.exit()